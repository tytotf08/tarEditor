// The Official distribution-ready Tar Editor, annotated
;!((lib = () => {}) => { // process the library function
	window.tar = lib(); // make it global
})(() => {
	...
	const autocomplete = [ 
		...
		{ open: "[", close: "]" },
		{ open: "(", close: ")" }
	]; // array with the autocomplete characters
	const tar = (wrap, ln = true, hl = (editor) => {editor.innerHTML = editor.innerHTML;}) => {
		if (!wrap) return; // no element
		let downcb = (e) => {}; // key down callback
		let upcb = (e) => {}; // key up callback
		let focused = true; // focused?
		let at = 0; // for undo
		let incarnations = [
			{html: "", pos: 0}
		]; // array with versions
		const saveCaretPosition = () => {
			const range = window.getSelection().getRangeAt(0); // selection
			let prefix = range.cloneRange();
			prefix.selectNodeContents(editor);
			prefix.setEnd(range.endContainer, range.endOffset); // get length of preceding text
			return prefix.toString().length; 
		}
		const restoreCaretPosition = (pos, context) => {
			for (const node of context.childNodes) { // go through nodes
				if (node.nodeType == Node.TEXT_NODE) { // main node?
					if (node.length >= pos) {
						// set
					...
					pos = restoreCaretPosition(pos, node); // recursive
					if (pos < 0) {
						return pos;
					}
				}
		...
		const beforeCursor = () => {
			...
			return r.toString(); // preceding text
		};
		const afterCursor = () => {
			const s = window.getSelection();
			const r0 = s.getRangeAt(0);
			...
			return r.toString(); // proceeding text
		};
		...
		if (wrap) {
			try {
				wrap.getAttribute("class"); // htmlel?
			} catch(e) {
		...
		if (ln === true) {
			wrap.appendChild(line_numbers); // normal
		} else {
			inputE = () => {
				prev = editor.textContent; // not normal
			};
			editor.style.paddingLeft = "1rem"; // pad
		}
			...
			if (e.key === "Tab") {
				...
				if (e.shiftKey) {
					if (hasLeadingTabs(editor)) {
						// move to after the tab, delete, restore caret
