...
const handleKey = function (e) {
	...
	for (i = 0; i < autoComplete.length; i++) {
		...
		if (
			e.key === "Backspace" &&
			beforeCursor().endsWith(autoComplete[i].open) &&
			afterCursor().startsWith(autoComplete[i].close)
		) {
			pos = saveCaretPosition(editor);
			restoreCaretPosition(pos + 1, editor);
			for (i = 0; i < 2; i++) {
	...
	if (e.key === "Tab") {
		...
		if (e.shiftKey) {
			...
			if (currentLine.startsWith("\t")) {
				...
				editor.textContent = html.replace(
					beforeCursor(editor),
					splice.join("\n")
				);
...
const isRedo = function (e) {
	return (
		(isCmd(e) && isShift(e) && e.key === "z") ||
		(isCmd(e) && !isShift(e) && e.key === "y")
	);
};
...
const getLines = function () {
	...
	document.querySelector("div#lines").innerHTML =
		"Lines: " + indexLines[indexLines.length - 1];
};
...
const close = function (e) {
	for (i = 0; i < document.querySelectorAll("div.tab").length; i++) {
		...
		if (
			document.querySelectorAll("div.tab")[i + 1].style.backgroundColor ===
			"rgb(15, 31, 104)"
		) {
...
const makeTab = function () {
	...
	tabEl.addEventListener("click", function (e) {
		...
		e.target.style.backgroundColor = "#0f1f68";
		incarnations = e.target.this.incarnations || {};
		after = e.target.this.afterCursor;
		at = e.target.this.at;
		editor.textContent = "";
		editor.focus();
		insertText(e.target.this.beforeCursor);
		pos = saveCaretPosition(editor);
		insertText(after);
		restoreCaretPosition(pos - after.length, editor);
	});
...
const update = function () {
	document.querySelectorAll("div.tab").forEach(function (el) {
		if (el.style.backgroundColor === "rgb(15, 31, 104)") {
			el.this = {
