...
const editor = tar(document.querySelector("main"), true, Prism.highlightElement);
...
langChooser.addEventListener("change", (e) => {
	...
	if (e.target.value.toLowerCase().indexOf("markdown") > -1) {
		editor.classList[2] = "language-markdown";
	} else if (e.target.value.toLowerCase().indexOf("css") > -1) {
		editor.classList[2] = "language-css";
	} else if (e.target.value.toLowerCase().indexOf("javascript") > -1) {
		editor.classList[2] = "language-javascript";
	} else {
		editor.classList[2] = "language-markup";
	}
});